...
#pragma warning(disable: 4996)//Den
#ifdef _DEBUG
                           ...
                           int nIconNormal)
{
	...
	if(nFlags & MF_POPUP)
	{
		...
		m_AllSubMenus.Add ((HMENU)(LONG_PTR)nID);
		m_SubMenus.Add ((HMENU)(LONG_PTR)nID);
		//Holden_20190109B_
	}
                           ...
                           int nIconNormal)
{
	...
	if(nFlags & MF_POPUP)
	{
		...
		m_AllSubMenus.Add ((HMENU)(LONG_PTR)nID);
		m_SubMenus.InsertAt (nPosition, (HMENU)(LONG_PTR)nID);
		//Holden_20190109B_
	}
	...
	do{
		...
		*/
		...
		if(dwFlags & MF_POPUP){
			...
			m_Stack[m_Stack.GetUpperBound ()]->AppendODMenuW (szCaption, uFlags,
				(UINT)(LONG_PTR)pSubMenu->m_hMenu, -1);
			//Holden_20190109B_
...
void BCMenu::SynchronizeMenu(void)
{
	...
	//Holden_20190109B_
	{
		...
		if(state&MF_POPUP)
		{
			...
			submenu = (UINT)(LONG_PTR)GetSubMenu (j)->m_hMenu;
			//Holden_20190109B_
		...
		else if(state&MF_SEPARATOR)
		{
			...
			if(!mdata)mdata=NewODMenu(j, state|MF_BYPOSITION|MF_SEPARATOR|MF_OWNERDRAW,0,_T(""));//SK: modified for Unicode correctness
			else 
...
BOOL BCMenu::AddBitmapToImageList(CImageList *bmplist,UINT nResourceID)
{
	...
	if (m_bDynIcons)
	{
		...
		bmplist->Add ((HICON)(LONG_PTR)nResourceID);
		//Holden_20190109B_
		bReturn=TRUE;
	}
	else{
		...
		else
		{
			...
			else
			{
				...
				//@@@@ HOLMES_20080808
					...
					if (m_MenuList[i]->nID == (UINT)(LONG_PTR)pSubMenu->m_hMenu)
					//Holden_20190109B_
					{
						delete m_MenuList.GetAt(i);
						m_MenuList.RemoveAt(i);
...
BCMenu* BCMenu::AppendODPopupMenuW(wchar_t *lpstrText)
{
	...
	AppendODMenuW (lpstrText, MF_POPUP, (UINT)(LONG_PTR)pSubMenu->m_hMenu, -1);
	//Holden_20190109B_
	return pSubMenu;
}
